						...
						<div class="row">
							<div class="col-sm-12">
								...
								<div class="btn-group mr10">
									<c:forEach items="${postRecords}" var="drecord" end="${exitFlag}">
										<c:if test="${drecord.userId == recruit.userId}">
											<button class='btn btn-primary'>
												<i class='fa fa-check mr5'></i>已经申请
											</button>
											<c:set var="exitFlag" value="0"></c:set>
										</c:if>
									</c:forEach>
									<c:if test="${exitFlag!=0}">
										...
										<div class="modal-content">
											...
											<div class="modal-footer">
												<button type="button" class="btn btn-default"
													data-dismiss="modal">取消申请</button>
												<button type="button" class="btn btn-primary"
													onclick="postResume('${recruit.recruitId}')"
													id="surePostostBtn">确认申请</button>
											</div>
										</div>
										<!-- /.modal-content -->
									</div>
										...
										<div class="about-list">
											<ul>
												<c:forEach items="${otherRecruits}" var="otherRecruit">
													<li><a
														href="recruit/detailRecruit.action?recruitId=${otherRecruit.recruitId}">${otherRecruit.title}</a></li>
												</c:forEach>
											</ul>
										</div>
									</div>
